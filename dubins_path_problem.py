...
class RRT_dubins_problem():
    ...
    class Node():
        ...
        def __init__(self, x, y, yaw, d_P, pseg_ind):
            self.x = x
            self.y = y
            self.path_x = []
            self.path_y = []
        ...
        def is_state_identical(self, node):
            ...
            """
            if abs(node.x - self.x) > 0.5:
                return False
            elif abs(node.y - self.y) > 0.5:
                return False
            elif abs(node.yaw - self.yaw) > 0.3:
                return False
    ...
    def __init__(self, start, goal, obstacle_list, map_area,
                 bias_path, bias_path_seg, max_iter=20000
                 ):
        """
        Init Parameter
        ...
        self.goal = self.Node(goal[0], goal[1], goal[2], 0, -1)
        ...
        self.bias_path_seg = bias_path_seg

        # self.curvature = 1.0 # for dubins path

    def rrt_planning(self,delta_ori,delta_R,seg_turn,sam_inter,sam_dens,bias_path,w_d, w_q, w_e, w_theta, display_map=False):
        """
            Calling function for your planning implementation
        """
        return rrt_planner(self,delta_ori,delta_R,seg_turn,sam_inter,sam_dens,bias_path,w_d, w_q, w_e, w_theta, display_map=display_map)
    ...
    #     source = np.array([from_node.x, from_node.y, from_node.yaw])
    #     target = np.array([to_node.x, to_node.y, to_node.yaw])
    #     direction = 1
    #     resolution = 0.1
    #     base = 0.4
    #     init_t = 0.1
    ...
    #     w_d = 0.5
    #     w_q = 0.5
    #     traj, _, _, inct, traj_cost = posq.integrate(source, target, direction,
    #                                        resolution, base,
    ...
    def draw_graph(self, bias_path):
        ...
        plt.plot(bias_path[0, :], bias_path[1, :], 'b')
        plt.axis([self.x_lim[0] - 2, self.x_lim[1] + 2, \
                    self.y_lim[0] - 2, self.y_lim[1] + 2])
        plt.grid(True)
        #plt.pause(0.001) # Required for Macs
...
def draw_horizontal_line(start_x, start_y, length, o_x, o_y):
    for i in range(start_x, start_x + length):
        for j in range(start_y, start_y + 1):
            o_x.append(i)
            o_y.append(j)

def draw_vertical_line(start_x, start_y, length, o_x, o_y):
    for i in range(start_x, start_x + 1):
        for j in range(start_y, start_y + length):
            o_x.append(i)
            o_y.append(j)

def run(delta_ori,delta_R,seg_turn,sam_inter,sam_dens,w_d, w_q, w_e, w_theta, choose_map, showplots):
    print("Executing: " + __file__)
#    ====Search Path with RRT====
    obstacleList = []  # [x,y,size(radius)]
    o_x, o_y = [], []
    draw_vertical_line(0, 0, 25, o_x, o_y)
    draw_vertical_line(25, 0, 26, o_x, o_y)
    draw_horizontal_line(0, 0, 25, o_x, o_y)
    draw_horizontal_line(0, 25, 25, o_x, o_y)

    # Choose which environment map to use
    ...
    if choose_map == 1:
        ...
        for x, y, l in zip(all_x, all_y, all_len):
            draw_vertical_line(x, y, l, o_x, o_y)
        start = [2.0, 2.0, np.deg2rad(30.0)]
        goal = [22.0, 22.0, np.deg2rad(90.0)]
        bias_path = np.array([[2, 4, 6, 9, 11, 14, 16, 19, 21, 22],
                              [2, 21, 21, 4, 4, 21, 21, 4, 4, 22]])
        # Narrow Corridor Map
    if choose_map == 2:
        # draw inner walls
        all_x = [7, 7, 10, 10, 22, 13, 19]
        all_y = [3, 10, 3, 10, 13, 13, 16]
        all_len = [5, 13, 5, 13, 10, 7, 3]
        for x, y, l in zip(all_x, all_y, all_len):
            draw_vertical_line(x, y, l, o_x, o_y)

        all_x[:], all_y[:], all_len[:] = [], [], []
        all_x = [0, 0, 10, 10, 13, 10, 13]
        all_y = [7, 10, 10, 7, 13, 22, 19]
        all_len = [8, 8, 13, 13, 10, 12, 7]
        for x, y, l in zip(all_x, all_y, all_len):
            draw_horizontal_line(x, y, l, o_x, o_y)

        # start = [2.0, 2.0, np.deg2rad(30.0)]
        start = [2.0, 8.0, np.deg2rad(30.0)]
        goal = [16.0, 16.0, np.deg2rad(90.0)]
        # bias_path = np.array([[2, 6, 8, 9, 23, 23, 12, 12, 20, 20, 18, 16],
        #                      [2, 2, 2, 8, 9, 11, 12, 20, 20, 15, 15, 16]])
        bias_path = np.array([[2, 23, 23, 12, 12, 20, 20, 18, 16],
                              [8, 9, 11, 12, 20, 20, 15, 15, 16]])

    for n in range(len(o_x)):
        obstacleList.append((o_x[n],o_y[n],0.4))

    # Set Initial parameters
    #start = [0.0, 0.0, np.deg2rad(90.0)]
    #goal = [20.0, 10.0, np.deg2rad(0.0)]
    map_area = [0, 25, 0, 25]
    #bias_path = np.array([[0, 10, 20], [0, 10, 10]])
    bias_path_seg = s_f.path_seg_config(bias_path)
    
    
    rrt_anyangle = RRT_dubins_problem(start = start, goal = goal, \
                                    obstacle_list = obstacleList, \
                                    map_area = map_area, \
                                    bias_path = bias_path, bias_path_seg = bias_path_seg, max_iter=4000)
    path_node_list, path_len_cost, path_curv_cost, path_time = rrt_anyangle.rrt_planning(delta_ori,delta_R,seg_turn,
                                                                                         sam_inter,sam_dens,bias_path,
                                                                                         w_d, w_q, w_e, w_theta,
                                                                                         display_map=False)
    path = get_path(path_node_list)
    print(f"Path length: {path_len_cost}")
    print(f"Path curvature cost: {path_curv_cost}")      
    ...
    if showplots:
        ...
        plt.plot(bias_path[0,:],bias_path[1,:],'b')
        plt.show()
    return path_node_list, path_len_cost, path_curv_cost, path_time
#
